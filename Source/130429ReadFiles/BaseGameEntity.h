...
protected:
  ...
  BaseGameEntity(int entity_type, Vector3D pos, double r):m_vPos(pos),
                                        m_dBoundingRadius(r),
                                        m_ID(NextValidID()),
                                        m_vScale(Vector3D(1.0,1.0, 1.0)),
                                        m_EntityType(entity_type),
...
public:
  ...
  void         SetPos(Vector3D new_pos){m_vPos = new_pos;}
  void		   SetX(float val) { m_vPos.x = val;}
  void		   SetY(float val) { m_vPos.y = val;}
  void		   SetZ(float val) { m_vPos.z = val;}
  ...
  Vector3D     Scale()const{return m_vScale;}
  void         SetScale(Vector3D val){m_dBoundingRadius *= MaxOf(val.x, val.y)/MaxOf(m_vScale.x, m_vScale.y); m_vScale = val;}
  void         SetScale(double val){m_dBoundingRadius *= (val/MaxOf(m_vScale.x, m_vScale.y)); m_vScale = Vector3D(val, val, val);} 
